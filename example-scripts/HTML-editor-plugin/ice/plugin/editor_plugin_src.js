(function() {
  tinymce.PluginManager.load(
    "icelib",
    tinymce.PluginManager.urls.ice + "/ice/ice.min.js",
    function() {
  ...
  tinymce.create("tinymce.plugins.IcePlugin", {
    ...
    init: function(ed, url) {
      ...
      ed.onInit.add(function(ed) {
        ...
        ed.serializer.addRules(
          self.insertTag + "[id|class|style|title|username|userid|cid]"
        );

        if (self.insertTag != self.deleteTag)
          ed.serializer.addRules(
            self.deleteTag + "[id|class|style|title|username|userid|cid]"
          );
        // Temporary tags to act as placeholders for deletes.
        ed.serializer.addRules("tempdel[data-allocation]");

        dom.loadCSS(
          self.css.indexOf("://") > 0 ? self.css : url + "/" + self.css
        );
      ...
      ed.addCommand("icecleanbody", function(el) {
        var body = changeEditor.getCleanContent(
          el || ed.getContent(),
          self.afterClean
        );
      ...
      ed.addCommand("iceaccept", function(node) {
        var n = node || ed.selection.getNode(),
          parents = [];
        ed.undoManager.add();
      ...
      ed.addCommand("iceacceptall", function() {
        var inserts = ed.dom.select(self.insertSelector),
          deletes = ed.dom.select(self.deleteSelector),
          parents = [];
      ...
      ed.addCommand("ice_strippaste", function(html) {
        return changeEditor.pluginsManager.plugins[
          "IceCopyPastePlugin"
        ].stripPaste(html);
      });
          ...
          if (isInsideChangeTag(node)) {
            menu.add({
              title:
                "<img src='" +
                url +
                "/img/ice-accept-change.png' style='vertical-align: middle;margin-left: -22px;margin-right: 5px;'>Accept Change",
              icon: "",
              cmd: "iceaccept"
            });
            menu.add({
              title:
                "<img src='" +
                url +
                "/img/ice-reject-change.png' style='vertical-align: middle;margin-left: -22px;margin-right: 5px;'>Reject Change",
              icon: "",
              cmd: "icereject"
            });
          }
      ...
      function isInsideChangeTag(n) {
        return !!ed.dom.getParent(
          n,
          self.insertSelector + "," + self.deleteSelector
        );
      }
      ...
      function cleanup(parents) {
        var empty = ed.dom.select(
          self.insertSelector + ":empty," + self.deleteSelector + ":empty"
        );
        parents = parents || [];
