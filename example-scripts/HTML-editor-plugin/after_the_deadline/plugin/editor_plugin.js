...
AtDCore.prototype.makeError = function(error_s, tokens, type, seps, pre) {
  ...
  if (new RegExp("\\b" + error_s + "\\b").test(error_s)) {
    ...
    struct.regexp = new RegExp(
      "(?!" + error_s + "<)" + error_s.replace(/\s+/g, seps) + "\\b"
    );
  } else if (new RegExp("\\b" + error_s).test(error_s)) {
    struct.regexp = new RegExp(
      "(?!" + error_s + "<)\\b" + error_s.replace(/\s+/g, seps)
    );
  } else {
    struct.regexp = new RegExp(
      "(?!" + error_s + "<)" + error_s.replace(/\s+/g, seps)
    );
  }
  struct.used = false;
  return struct;
...
AtDCore.prototype.processXML = function(responseXML) {
  ...
  for (var i = 0; i < errors.length; i++) {
    if (errors[i].getElementsByTagName("string").item(0).firstChild != null) {
      var errorString = errors[i].getElementsByTagName("string").item(0)
        .firstChild.data;
      var errorType = errors[i].getElementsByTagName("type").item(0).firstChild
        .data;
      var errorDescription = errors[i]
        .getElementsByTagName("description")
        .item(0).firstChild.data;
      var errorContext;
      if (
        errors[i].getElementsByTagName("precontext").item(0).firstChild != null
      )
        errorContext = errors[i].getElementsByTagName("precontext").item(0)
          .firstChild.data;
      else errorContext = "";
      if (this.ignore_strings[errorString] == undefined) {
        ...
        if (
          errors[i].getElementsByTagName("suggestions").item(0) != undefined
        ) {
          var suggestions = errors[i]
        ...
        if (errors[i].getElementsByTagName("url").item(0) != undefined) {
          var errorUrl = errors[i].getElementsByTagName("url").item(0)
            .firstChild.data;
        ...
        if (errorDescription == "Spelling")
          suggestion["description"] = this.getLang(
            "menu_title_spelling",
            "Spelling"
          );
        if (errorDescription == "Repeated Word")
          suggestion["description"] = this.getLang(
            "menu_title_repeated_word",
            "Repeated Word"
          );
        if (errorDescription == "Did you mean...")
          suggestion["description"] = this.getLang(
            "menu_title_confused_word",
            "Did you mean..."
          );
      }
    }
  ...
  if (ecount > 0)
    errorStruct = this.buildErrorStructure(
      spellingErrors,
      enrichment,
      grammarErrors
    );
...
AtDCore.prototype.findSuggestion = function(element) {
  ...
  var context = (this.getAttrib(element, "pre") + "").replace(
    /[\\,!\\?\\."\s]/g,
    ""
  );
...
TokenIterator.prototype.peek = function(n) {
  ...
  for (var x = this.index; x < end; x++) peepers.push(this.tokens[x]);
  return peepers;
};
          ...
          var bringTheHurt = function(node) {
            if (node.nodeType == 3) {
              ...
              if (
                ...
                return parent.create(
                  '<span class="mceItemHidden">&nbsp;</span>' +
                    node.nodeValue
                      .substr(1, node.nodeValue.length - 1)
                      .replace(regexp, result),
                  false
                );
              else
                return parent.create(
                  node.nodeValue.replace(regexp, result),
                  false
                );
            } else {
              ...
              for (var y = 0; y < contents.length; y++) {
                if (
                  contents[y].nodeType == 3 &&
                  regexp.test(contents[y].nodeValue)
                ) {
                  var nnode;
                  if (
                    parent.isIE() &&
                    ...
                    nnode = parent.create(
                      contents[y].nodeValue.replace(regexp, result),
                      true
                    );
...
(function() {
  ...
  tinymce.create("tinymce.plugins.AfterTheDeadlinePlugin", {
    ...
    init: function(ed, url) {
          ...
          function(data, request, someObject) {
            ...
            if (
              ...
              ed.windowManager.alert(
                plugin.editor.getLang(
                  "AtD.message_server_error",
            ...
            if (
              request.responseXML.getElementsByTagName("message").item(0) !=
              null
            ) {
              ed.windowManager.alert(
                request.responseXML.getElementsByTagName("message").item(0)
            ...
            if (ecount == 0 && (!callback || callback == undefined))
              ed.windowManager.alert(
                plugin.editor.getLang(
                  "AtD.message_no_errors_found",
                  "No writing errors were found."
                )
              );
            else if (callback) callback(ecount);
          }
        );
      });
      editor.onInit.add(function() {
        if (editor.settings.content_css !== false) {
          editor.dom.loadCSS(
            editor.getParam("atd_css_url", url + "/css/content.css")
          );
        }
      });
    ...
    createControl: function(name, controlManager) {
      ...
      if (name == "AtD") {
        return controlManager.createButton(name, {
          title: this.editor.getLang(
            "AtD.button_proofread_tooltip",
            "Proofread Writing"
          ),
          image: this.editor.getParam(
            "atd_button_url",
            this.url + "/atdbuttontr.gif"
          ),
    ...
    markMyWords: function(errors) {
      ...
      var se = ed.selection,
        b = se.getBookmark();
      var ecount = ed.core.markMyWords(
        ed.core.contents(this.editor.getBody()),
        errors
      );
      se.moveToBookmark(b);
      return ecount;
      ...
      if (ed.core.isMarkedNode(e.target)) {
        ...
        if (
          ...
          (function(url) {
            m.add({
              title: plugin.editor.getLang(
                "AtD.menu_option_explain",
                "Explain..."
              ),
              onclick: function() {
                ed.windowManager.open(
                  { url: url, width: 480, height: 380, inline: true },
                  { theme_url: this.url }
                );
              }
            });
        ...
        if (
          String(this.editor.getParam("atd_ignore_enable", "false")) == "true"
        ) {
          m.add({
            title: plugin.editor.getLang(
              "AtD.menu_option_ignore_always",
              "Ignore always"
            ),
            onclick: function() {
              ...
              if (url == "{backend}") {
                ...
                tinymce.util.Cookie.setHash(
                  "atd_ignore",
                  ignore,
                  new Date(new Date().getTime() + 157680000000)
                );
              } else {
                ...
                tinymce.util.XHR.send({
                  url:
                    url +
                    encodeURI(e.target.innerHTML).replace(/&/g, "%26") +
                    "&key=" +
                    id,
