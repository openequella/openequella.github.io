      ...
      if (this.ignore_strings[errorString] == undefined) {
        ...
        if (errorDescription == "Cliches")
          suggestion["description"] =
            "Clich&eacute;s"; /* done here for backwards compatability with current user settings */
...
AtDCore.prototype.markMyWords = function(container_nodes, errors) {
  ...
  this.map(nl, function(n) {
    ...
    if (n.nodeType == 3) {
      v =
        n.nodeValue; /* we don't want to mangle the HTML so use the actual encoded string */
      var tokens = n.nodeValue.split(
        seps
